...
```{r}
m <- run_sdmTMB(dat=trawl_roms_utm,spp="yelloweye rockfish",use_depth=F,include_substrate=T) # all other options default
```
...
```{r}
preds.yelloweye = make_predictions(m, gcm = "hadl")
```

```{r}
write.csv(preds.yelloweye, "yelloweye.csv", row.names=FALSE, quote=FALSE) 
```
...
```{r}
make_six_maps <- function(spp_preds){
  p1 <- map_year(spp_preds,yr_vec=c(1985:2010))+labs(title='baseline')
  p2 <- map_year(spp_preds,yr_vec=c(2045:2050))+labs(title='mid century1')
  p3 <- map_year(spp_preds,yr_vec=c(2055:2060))+labs(title='mid century2')
  p4 <- map_year(spp_preds,yr_vec=c(2065:2070))+labs(title='mid century3')
  p5 <- map_year(spp_preds,yr_vec=c(2080:2090))+labs(title='end of century1')
    p6 <- map_year(spp_preds,yr_vec=c(2090:2100))+labs(title='end of century2')
...
```{r}
make_six_diff_maps <- function(spp_preds){
  p1 <- make_comparison_map(spp_preds,yr=c(2040:2050))+labs(title='mid century')
  p2 <- make_comparison_map(spp_preds,yr=c(2050:2060))+labs(title='mid century1')
  p3 <- make_comparison_map(spp_preds,yr=c(2060:2070))+labs(title='mid century')
  p4 <- make_comparison_map(spp_preds,yr=c(2070:2080))+labs(title='end of century')
  p5 <- make_comparison_map(spp_preds,yr=c(2080:2090))+labs(title='end of century1')
  p6 <- make_comparison_map(spp_preds,yr=c(2090:2100))+labs(title='end of century2')
  plot_grid(p1,p2,p3,p4,p5,p6,nrow=2)
}
```

```{r,fig.height=5,fig.width=5.3}
# make_six_diff_maps(mp)
```
...
```{r}
process_species_model <- function(dat=trawl_roms_utm,species,incl_depth,incl_substrate,dir.out=here::here('Janelle')){
