...
```{r}
m <- run_sdmTMB(dat=trawl_roms_utm,spp="dover sole",use_depth=F,env_spline=T,include_substrate=F)
summary(m)
```
...
```{r}
spp_dat$pred = predict(m)
spp_dat$resid = residuals(m)
```

```{r}
pred_space(df = spp_dat, time = "year")
```
...
```{r}
map_year(preds,yr_vec = c(1980:2020))
```
