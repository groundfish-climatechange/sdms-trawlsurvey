...
```{r}
run_sdmTMB_cv <- function(dat,spp,nknots=400,use_depth=T,time_vary=F,spatial_field=T,include_substrate=F,hab_spline=F,env_spline=F,spline_k=3,return_what='loglik'){
...
```{r}
m <- run_sdmTMB_cv(dat=trawl_roms_utm,spp="sablefish",use_depth=T,env_spline=T,include_substrate=F)
summary(m)
```