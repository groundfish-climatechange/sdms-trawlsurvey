...
```{r}
process_species_model <- function(dat=trawl_roms_utm,species,incl_depth,incl_substrate,dir.out=here::here('model output')){
...
Longnose skate: depth and substrate
```{r}
process_species_model(species='longnose skate',incl_depth = T,incl_substrate = T)
```

Big skate: depth and substrate
```{r}
process_species_model(species='big skate',incl_depth = T,incl_substrate = T)
```
Pacific spiny dogfish: temperature and oxygen only
```{r}
process_species_model(species='pacific spiny dogfish',incl_depth = F,incl_substrate = F)
```

Spotted ratfish: depth
```{r}
...
Curlfin sole: substrate
```{r}
process_species_model(species='curlfin sole',incl_depth = F,incl_substrate = T)
```
...
Slender sole: depth and substrate
```{r}
process_species_model(species='slender sole',incl_depth = T,incl_substrate = T)
```

Petrale sole: substrate
```{r}
process_species_model(species='petrale sole',incl_depth = F,incl_substrate = T)
```

English sole: depth and substrate
```{r}
process_species_model(species='english sole',incl_depth = T,incl_substrate = T)
```

Arrowtooth flounder: depth and substrate
```{r}
process_species_model(species='arrowtooth flounder',incl_depth = T,incl_substrate = T)
```

Pacific sanddab: substrate
```{r}
process_species_model(species='pacific sanddab',incl_depth = F,incl_substrate = T)
```

## Rockfish
Aurora rockfish: depth and substrate
```{r}
process_species_model(species='aurora rockfish',incl_depth = T,incl_substrate = T)
```

Bocaccio rockfish: substrate
```{r}
process_species_model(species='bocaccio',incl_depth = F,incl_substrate = T)
```

Canary rockfish: substrate
```{r}
process_species_model(species='canary rockfish',incl_depth = F,incl_substrate = T)
```

Chilipepper rockfish: substrate
```{r}
process_species_model(species='chilipepper',incl_depth = F,incl_substrate = T)
```

Darkblotched rockfish: depth and substrate
```{r}
process_species_model(species='darkblotched rockfish',incl_depth = T,incl_substrate = T)
```

Greenstriped rockfish: substrate
```{r}
process_species_model(species='greenstriped rockfish',incl_depth = F,incl_substrate = T)
```

Pacific ocean perch: substrate
```{r}
process_species_model(species='pacific ocean perch',incl_depth = F,incl_substrate = T)
```

Sharpchin rockfish: substrate
```{r}
process_species_model(species='sharpchin rockfish',incl_depth = F,incl_substrate = T)
```

Shortbelly rockfish: substrate
```{r}
process_species_model(species='shortbelly rockfish',incl_depth = F,incl_substrate = T)
```

Shortraker rockfish: substrate
```{r}
process_species_model(species='shortraker rockfish',incl_depth = F,incl_substrate = T)
```

Splitnose rockfish: depth and substrate
```{r}
process_species_model(species='splitnose rockfish',incl_depth = T,incl_substrate = T)
```

Stripetail rockfish: temperature and oxygen only
```{r}
...
```{r}
process_species_model(species='widow rockfish',incl_depth = F,incl_substrate = T)
```

Yelloweye rockfish: substrate
```{r}
process_species_model(species='yelloweye rockfish',incl_depth = F,incl_substrate = T)
```

Yellowtail rockfish: substrate
```{r}
process_species_model(species='yellowtail rockfish',incl_depth = F,incl_substrate = T)
```

## Other

Lingcod: depth and substrate
```{r}
process_species_model(species='lingcod',incl_depth = T,incl_substrate = T)
```

Longspine thornyhead: depth and substrate
```{r}
process_species_model(species='longspine thornyhead',incl_depth = T,incl_substrate = T)
```

Pacific grenadier: depth
```{r}
process_species_model(species='pacific grenadier',incl_depth = T,incl_substrate = F)
```

Sablefish: temperature and oxygen only
```{r}
process_species_model(species='sablefish',incl_depth = F,incl_substrate = F)
```

Shortspine thornyhead: substrate
```{r}
process_species_model(species='shortspine thornyhead',incl_depth = F,incl_substrate = T)
```
