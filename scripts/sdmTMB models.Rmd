...
```{r}
model_species <- function(spp,use_substrate=T){
  ...
  if(!use_substrate) {
    ...
    out <- models_to_run %>% 
      mutate(model=purrr::pmap(list(spp=spp,spatial_field=spatial_field,env_spline=env_spline,include_substrate=include_substrate),run_sdmTMB,dat=trawl_roms_utm)) %>%
      mutate(model_cv=purrr::pmap(list(spp=spp,spatial_field=spatial_field,env_spline=env_spline,include_substrate=include_substrate),run_sdmTMB_cv,dat=trawl_roms_utm,return_what="model"))
