...
Try it out with different options

No spatial field, linear environment

```{r}
sable1 <- run_sdmTMB(sablefish_dat,spp="sablefish",nknots=400,use_depth=F,time_varying=F,spatial_field=F,hab_spline=F,env_spline=F,spline_k=3)
```

No spatial field, wiggly environment

```{r}
sable2 <- run_sdmTMB(sablefish_dat,spp="sablefish",nknots=400,use_depth=F,time_varying=F,spatial_field=F,hab_spline=F,env_spline=T,spline_k=3)
```

Spatial field, linear environment

```{r}
sable3 <- run_sdmTMB(sablefish_dat,spp="sablefish",nknots=400,use_depth=F,time_varying=F,spatial_field=T,hab_spline=F,env_spline=F,spline_k=3)
```

Spatial field, wiggly environment

```{r}
sable4 <- run_sdmTMB(sablefish_dat,spp="sablefish",nknots=400,use_depth=F,time_varying=F,spatial_field=T,hab_spline=F,env_spline=T,spline_k=3)
```
