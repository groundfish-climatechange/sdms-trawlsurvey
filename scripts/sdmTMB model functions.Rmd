...
plot_depth_distribution2 <- function(ens_preds,start_year=2020,end_year=2100){
  ...
  y %>% 
    ...
    # scale_color_manual(values=c("#2271B2","#d55e00"))+
    labs(x="Year",y="Proportion < 700 fathoms",title="",col="Species")+
...
```{r}
# right now this works on a group of ensemble outputs
plot_dist_to_shore_change <- function(multispp_df,include_legend=T){
  d <- multispp_df %>%  
    left_join(roms_dist_to_coast,by=c("longitude","latitude")) %>% 
    ...
    mutate(change_from_2020=w.dist-w.dist[year==2020]) %>% 
    ungroup() %>% 
    filter(year==2100) %>% 
    mutate(spp_plotting=case_when(
      species=='dover' ~ "Dover Sole",
