...
make_predictions <- function(modelobj,gcm="hadl"){
  ...
  newdata <- roms %>% 
    ...
    drop_na() %>% 
    mutate(depth_m= -depth_m) %>%  # THIS IS VERY IMPORTANT FOR MODELS THAT INCLUDE DEPTH
...
```{r}
# plot_dist_to_shore_change <- function(multisims_df,include_legend=T){
#   d <- multisims_df %>%  
#     left_join(roms_dist_to_coast,by=c("longitude","latitude")) %>% 
#     group_by(species,year,lat) %>% 
#     mutate(totest = sum(exp(mean_est)),
#            lower=sum(exp(est5)),
#            upper=sum(exp(est95))) %>% 
#     mutate(rel_est=exp(mean_est)/totest,
#            rel_est_lower=exp(est5)/lower,
#            rel_est_upper=exp(est95)/upper) %>% 
#     mutate(w.dist=rel_est*km_to_coast,
#            w.dist.lower=rel_est_lower*km_to_coast,
#            w.dist.upper=rel_est_upper*km_to_coast) %>% 
#     summarise(w.dist=sum(w.dist),
#               w.dist.lower=sum(w.dist.lower),
#               w.dist.upper=sum(w.dist.upper)) %>% 
#     ungroup() 
#   # 1985-2010 means
#   d2 <- d %>% 
#     filter(year %in% c(1985:2010)) %>% 
#     group_by(species,lat) %>% 
#     summarise(w.dist.hist=mean(w.dist),
#               w.dist.hist.low=mean(w.dist.lower),
#               w.dist.hist.high=mean(w.dist.upper)) %>% 
#     ungroup()
#   
#   # 2075-2100 means
#   d3 <-  d %>% 
...
#     group_by(species,lat) %>% 
#     summarise(w.dist.fut=mean(w.dist),
#               w.dist.fut.low=mean(w.dist.lower),
#               w.dist.fut.high=mean(w.dist.upper)) %>% 
#     ungroup()
#   
#   # difference
#   
#   d4 <- d2 %>% 
#     left_join(d3,by=c('species','lat'))%>% 
#     mutate(change1=w.dist.fut-w.dist.hist,
#            change2=w.dist.fut.low-w.dist.hist.low,
#            change3=w.dist.fut.high-w.dist.hist.high) %>% 
#     # mutate(low=pmin(change1,change2,change3),
#     #        high=pmax(change1,change2,change3)) %>% 
#     mutate(spp_plotting=case_when(
#       species=='dover' ~ "Dover Sole",
#       species=="ls" ~ "Longspine",
#       species=="ss"~"Shortspine",
...
#     )) %>% 
#     mutate(spp_plotting=factor(spp_plotting,levels=c("Dover Sole","Sablefish","Shortspine","Longspine"))) %>% 
#     dplyr::select(species,lat,spp_plotting,contains("change")) %>% 
#     pivot_longer(contains("change"),names_to="obs",values_to="w.dist.change")
#   
#   if(include_legend){
#       p <- d4 %>% 
#         ggplot(aes(x=lat,y=w.dist.change,col=spp_plotting))+
...
#         coord_flip()+
#         # theme(legend.position = c(0.8,0.7))+
#         labs(y="Change in Distance from Shore (km)",x="Latitude",col="Species")+
#         theme(panel.background = element_rect(fill=NA,color='black'))
#   } else {
#       p <- d4 %>% 
#         ggplot(aes(x=lat,y=w.dist.change,col=spp_plotting))+
...
#         # scale_y_reverse(limits=c(90,20))+
#         coord_flip()+
#         labs(y="Change in Distance from Shore (km)",x="Latitude")+
#         theme(panel.background = element_rect(fill=NA,color='black'))
#   }
# 
#   p
