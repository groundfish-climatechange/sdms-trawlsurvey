...
run_sdmTMB <- function(dat,spp,nknots=400,use_depth=F,time_vary=F,spatial_field=T,hab_spline=F,env_spline=F,spline_k=3){
...
```{r}
run_sdmTMB_cv <- function(dat,spp,nknots=400,use_depth=F,time_vary=F,spatial_field=T,hab_spline=F,env_spline=F,spline_k=3,return_what='loglik'){
  # filter data for species
  modeldat <- prepare_species(dat,spp=spp)
...
```{r,eval=F}
test_cv <- run_sdmTMB_cv(dat=trawl_roms_utm,spp='sablefish')
```
...
```{r eval=FALSE}
the_data <- sf::st_as_sf(modeldat, coords = c("longitude", "latitude"),crs="+proj=utm +zone=10 +datum=WGS84 +units=km")
sb <- spatialBlock(
