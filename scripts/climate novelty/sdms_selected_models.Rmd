---
title: "Selected SDMs for West Coast Groundfish"
author: "Owen Liu"
date: '2023-10-11'
...
library(vista) # for some output plotting
```

# Introduction

Running SDMs using `sdmTMB` for California Current elasmobranchs, rockfish, flatfish, and other key species.
...
run_sdmTMB <- function(dat,spp,use_depth=F,time_vary=F,spatial_field=F,include_substrate=T,hab_spline=T,env_spline=T,spline_k=3){
  ...
  #extra time needed to make room for predictions?
  et<-setdiff(1980:2100,unique(year(trawl_roms_utm$date)))
...
```{r,eval=F}
m <- run_sdmTMB(dat=trawl_roms_utm,spp="longnose skate",use_depth=T,include_substrate=F) # all other options default
```

## Diagnostic Plots

Functions to produce QQ plots, spatiotemporal predictions, and spatiotemporal residuals

```{r}
make_diagnostic_plots <- function(m,dat=trawl_roms_utm){
  spp=unique(m$data$species)
  sppdat <- prepare_species(dat,spp)
...
Function to make predictions of future species distributions. Here, we choose from on of three global climate model projections (Hadley=`hadl`, GFDL=`gfdl`,IPSL=`ipsl`)
...
```{r,fig.height=5,fig.width=5.3,eval=F}
...
# mpred <- predict(m)
# ?ggpredict
# x <- ggeffects::ggpredict(m,"depth_trawl_norm [-0.5:0.5,by=0.1]")
# plot(x)
# 
# original_predvar = str_sub("depth_trawl_norm",end=-6L)
# 
# predvar_dat=m$data %>% dplyr::select(all_of(original_predvar)) %>% pluck(1)
# mean_predvar <- mean(predvar_dat,na.rm=T)
# sd_predvar <- sd(predvar_dat,na.rm=T)
# predvar_trans <- (x %>% pluck(1))*sd_predvar+mean_predvar
# 
...
plot_single_env_relationship <- function(fit,predvar="depth_trawl_norm"){
  ...
  x <- visreg(fit=fit,xvar=predvar,scale='response',plot=FALSE,type='conditional')
  datfit <- x$fit
...
```{r}
process_species_model <- function(dat=trawl_roms_utm,species,incl_depth,incl_substrate,dir.out=here::here('model output','climate novelty','roms')){
  tic(paste(species,"processing total time"))
  tic(paste("Fitting model for",species))
  # run model
  m <- run_sdmTMB(dat = dat,spp=species,use_depth=incl_depth,time_vary = F,spatial_field = F,include_substrate = incl_substrate,hab_spline = T,env_spline = T,spline_k = 3)
  toc()
  
  # make diagnostic plots
  tic(paste("Making diagnostic plots for",species))
  diagplots <- make_diagnostic_plots(m,dat=dat)
  toc()
...
As of 10.10.2023, we have decided for this particular project to model each species with consistent covariates (temperature, oxygen, and depth), to facilitate comparison between them.

## Elasmobranchs

Longnose skate
```{r,message=F}
process_species_model(species='longnose skate',incl_depth = T,incl_substrate = F)
```

Big skate
```{r}
process_species_model(species='big skate',incl_depth = T,incl_substrate = F)
```
Pacific spiny dogfish
```{r}
process_species_model(species='pacific spiny dogfish',incl_depth = T,incl_substrate = F)
```

Spotted ratfish
```{r}
process_species_model(species='spotted ratfish',incl_depth = T,incl_substrate = F)
```
...
Dover sole
```{r}
process_species_model(species='dover sole',incl_depth = T,incl_substrate = F)
```

Curlfin sole
```{r}
process_species_model(species='curlfin sole',incl_depth = T,incl_substrate = F)
```

Rex sole
```{r}
process_species_model(species='rex sole',incl_depth = T,incl_substrate = F)
```

Slender sole
```{r}
process_species_model(species='slender sole',incl_depth = T,incl_substrate = F)
```

Petrale sole
```{r}
process_species_model(species='petrale sole',incl_depth = T,incl_substrate = F)
```

English sole
```{r}
process_species_model(species='english sole',incl_depth = T,incl_substrate = F)
```

Arrowtooth flounder
```{r}
process_species_model(species='arrowtooth flounder',incl_depth = T,incl_substrate = F)
```

Pacific sanddab
```{r}
process_species_model(species='pacific sanddab',incl_depth = T,incl_substrate = F)
```

## Rockfish
Aurora rockfish
```{r}
process_species_model(species='aurora rockfish',incl_depth = T,incl_substrate = F)
```

Bocaccio rockfish- depth-restricted model

```{r}
# process_species_model(species='bocaccio',incl_depth = T,incl_substrate = F)
# crop to areas <= 500m depth
# tempdat <- prepare_species(trawl_roms_utm,"bocaccio")
# # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #333m

trawl_depthcrop_500m <- trawl_roms_utm %>%
  filter(depth_trawl<=500)
# 
process_species_model(dat=trawl_depthcrop_500m,species = "bocaccio",incl_depth=T,incl_substrate = F)
```

Canary rockfish- depth-restricted model
```{r}
# process_species_model(species='canary rockfish',incl_depth = T,incl_substrate = F)
# tempdat <- prepare_species(trawl_roms_utm,"canary rockfish")
# # # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #264m
process_species_model(dat=trawl_depthcrop_500m,species = "canary rockfish",incl_depth=T,incl_substrate = F)
```

Chilipepper rockfish
```{r}
process_species_model(species='chilipepper',incl_depth = T,incl_substrate = F)
```

Darkblotched rockfish
```{r}
process_species_model(species='darkblotched rockfish',incl_depth = T,incl_substrate = F)
```

Greenstriped rockfish
```{r}
process_species_model(species='greenstriped rockfish',incl_depth = T,incl_substrate = F)
```

Pacific ocean perch
```{r}
process_species_model(species='pacific ocean perch',incl_depth = T,incl_substrate = F)
```

Sharpchin rockfish - depth-restricted model
```{r}
# tempdat <- prepare_species(trawl_roms_utm,"sharpchin rockfish")
# # # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #454m

process_species_model(dat = trawl_depthcrop_500m, species='sharpchin rockfish',incl_depth = T,incl_substrate = F)
```

Shortbelly rockfish - depth-restricted model 

```{r}
# tempdat <- prepare_species(trawl_roms_utm,"shortbelly rockfish")
# # # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #406m
process_species_model(dat=trawl_depthcrop_500m,species='shortbelly rockfish',incl_depth = T,incl_substrate = F)
```

Shortraker rockfish
```{r}
# tempdat <- prepare_species(trawl_roms_utm,"shortraker rockfish")
# # # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #458m
process_species_model(dat=trawl_depthcrop_500m, species='shortraker rockfish',incl_depth = T,incl_substrate = F)
```

Splitnose rockfish
```{r}
process_species_model(species='splitnose rockfish',incl_depth = T,incl_substrate = F)
```

Stripetail rockfish
```{r}
# tempdat <- prepare_species(trawl_roms_utm,"stripetail rockfish")
# # # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #435m
process_species_model(dat=trawl_depthcrop_500m,species='stripetail rockfish',incl_depth = T,incl_substrate = F)
```

Widow rockfish
```{r}
process_species_model(species='widow rockfish',incl_depth = T,incl_substrate = F)
```

Yelloweye rockfish
```{r}
# tempdat <- prepare_species(trawl_roms_utm,"yelloweye rockfish")
# # # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #250m
# trawl_depthcrop_300m <- trawl_roms_utm %>% filter(depth_trawl<=300)
process_species_model(dat=trawl_depthcrop_300m,species='yelloweye rockfish',incl_depth = T,incl_substrate = F)
```

Yellowtail rockfish
```{r}
# tempdat <- prepare_species(trawl_roms_utm,"yellowtail rockfish")
# # # check deepest observed depth
# tempdat %>% filter(cpue_kg_km2>0) %>% pull(depth_trawl) %>% max() #343m
process_species_model(dat=trawl_depthcrop_500m,species='yellowtail rockfish',incl_depth = T,incl_substrate = F)
```
...
Lingcod
```{r}
process_species_model(species='lingcod',incl_depth = T,incl_substrate = F)
```

Longspine thornyhead
```{r}
process_species_model(species='longspine thornyhead',incl_depth = T,incl_substrate = F)
```

Pacific grenadier
```{r}
process_species_model(species='pacific grenadier',incl_depth = T,incl_substrate = F)
```

Sablefish
```{r}
process_species_model(species='sablefish',incl_depth = T,incl_substrate = F)
```

Shortspine thornyhead
```{r}
process_species_model(species='shortspine thornyhead',incl_depth = T,incl_substrate = F)
```
...
```{r,eval=F}
# try bocaccio with depth
bocdat <- prepare_species(trawl_roms_utm,"bocaccio")
meshcheck <- make_mesh(bocdat,xy_cols = c('longitude','latitude'), 
                   cutoff = 20)
# crop to general areas that have had positive observations at some point in the past
boccrop <- bocdat %>% 
  filter(cpue_kg_km2>0)
# max observed depth for bocaccio is 333m
# crop to areas <= 500m depth?
boc_depthcrop <- bocdat %>% 
  filter(depth_trawl<=500)

meshcheck <- make_mesh(boc_depthcrop,xy_cols = c('longitude','latitude'), 
                   cutoff = 20)
...
```{r,eval=F}
# try modeling with these different datasets
m1 <- run_sdmTMB(dat=trawl_roms_utm,spp="bocaccio",use_depth=T,include_substrate=F) 
m2 <- run_sdmTMB(dat=boc_depthcrop,spp="bocaccio",use_depth=T,include_substrate=F) # this worked fine!
x <- make_diagnostic_plots(m=m2,dat=trawl_depthcrop_500m)
```

```{r blackrf, eval=F}
x <- prepare_species(trawl_roms_utm,"black rockfish")
y <- x %>% filter(cpue_kg_km2>0)
```
