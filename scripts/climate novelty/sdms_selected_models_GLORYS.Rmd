---
title: "Selected SDMs for West Coast Groundfish using GLORYS"
author: "Owen Liu"
date: '2023-10-11'
...
# Load Data
```{r}
# rmarkdown::render(here::here('scripts','sdmTMB-data-construction-glorys.Rmd'),quiet=TRUE)
trawl <- read_rds(here('data','trawl_glorys_joined.rds'))

trawl_norm <- trawl %>%  # rescale depth, oxygen, and temp to be N(0,1)
...
run_sdmTMB <- function(dat,spp,use_depth=F,time_vary=F,spatial_field=F,include_substrate=T,hab_spline=T,env_spline=T,spline_k=3){
  ...
  # make spde
  spde <- make_mesh(modeldat,xy_cols = c('lontrawlUTM','lattrawlUTM'), 
                   cutoff = 20)
...
```{r,eval=F}
m <- run_sdmTMB(dat=trawl_norm,spp="longnose skate",use_depth=T,include_substrate=T) # all other options default
```
...
make_diagnostic_plots <- function(m){
  ...
  sppdat <- prepare_species(trawl_norm,spp)
  ...
  sppdat$pred <- preds
  preds_time_space <- pred_space(sppdat,X='lontrawlUTM',Y='lattrawlUTM',time='year') + coord_equal()
  preds_space <- pred_space(sppdat,X='lontrawlUTM',Y='lattrawlUTM',time='year',by_time = F)
  
  # spatiotemporal residuals
  resids_space <- resid_space(sppdat,X='lontrawlUTM',Y='lattrawlUTM',time='year') + coord_equal()
...
make_predictions <- function(modelobj,gcm="hadl"){
  ...
  temperature <- newdata %>% dplyr::select(contains('bt')) %>% 
    ...
    mutate(thetao=(temperature-mean_t)/sd_t)
  
  oxygen <- newdata %>% dplyr::select(contains('oxy')) %>% 
    set_names('oxygen') %>% 
    mutate(o2=(oxygen-mean_oxy)/sd_oxy)
...
plot_all_env_relationships <- function(m){
  ...
  if(grepl('temp_roms',form)){
    temp_relationship <- plot_single_env_relationship(m,predvar="thetao")+
      labs(x="Bottom Temperature")
  } else temp_relationship=NULL
  
  if(grepl('o2',form)){
    oxy_relationship <- plot_single_env_relationship(m,predvar="o2")+
...
Bocaccio rockfish
```{r}
process_species_model(species='bocaccio',incl_depth = F,incl_substrate = F)
```

Canary rockfish
```{r}
process_species_model(species='canary rockfish',incl_depth = F,incl_substrate = F)
```

Chilipepper rockfish
```{r}
process_species_model(species='chilipepper',incl_depth = F,incl_substrate = F)
```
...
Sharpchin rockfish
```{r}
process_species_model(species='sharpchin rockfish',incl_depth = F,incl_substrate = F)
```

Shortbelly rockfish
```{r}
process_species_model(species='shortbelly rockfish',incl_depth = F,incl_substrate = F)
```

Shortraker rockfish
```{r}
process_species_model(species='shortraker rockfish',incl_depth = F,incl_substrate = T)
```
...
Stripetail rockfish
```{r}
process_species_model(species='stripetail rockfish',incl_depth = F,incl_substrate = F)
```
...
Yelloweye rockfish
```{r}
process_species_model(species='yelloweye rockfish',incl_depth = F,incl_substrate = F)
```

Yellowtail rockfish
```{r}
process_species_model(species='yellowtail rockfish',incl_depth = F,incl_substrate = F)
```
