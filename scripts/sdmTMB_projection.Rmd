...
ensemble_predictions <- function(model_df,gcm='hadl'){
  ...
  ens_preds <- all_predictions %>% 
    group_by(year,longitude,latitude,lat,lon) %>% 
...
map_year <- function(model_ens,yr_vec=c(2000),return_pred_df=F){
...
make_index <- function(model_df){
  ...
  predictions_list <- model_df %>% pluck('model') %>% 
    furrr::future_map(make_predictions) 
  toc()
