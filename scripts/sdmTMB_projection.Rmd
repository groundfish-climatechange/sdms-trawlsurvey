...
make_predictions <- function(modelobj,gcm="hadl",return_tmb_object=F){
  ...
  predicted_cpue_km2 <- predict(modelobj,newdata,return_tmb_object=ifelse(return_tmb_object,T,F))
...
sable_future %>% 
  group_by(year) %>% 
  summarise(mean_est=mean(est,na.rm=T),sd_est=sd(est,na.rm=T)) %>% 
  ggplot(aes(year,mean_est))+
  geom_line()+
  labs(x="year",y="mean cpue")
```
...
```{r}
# defunct: want to us sdmTMB::get_cog instead
# calc_cog <- function(df,makeplot=T){
#   cog_ts <- df %>% 
#     group_by(year) %>% 
#     summarise(cogx=weighted.mean(longitude,est,na.rm=T),
#               cogy=weighted.mean(latitude,est,na.rm=T)) %>% 
#     ungroup()
#   
#   if(makeplot) {
#     d<-cog_ts %>% 
#       st_as_sf(coords=c("cogx","cogy"),crs = "+proj=utm +zone=10 +datum=WGS84 +units=km",remove=F)
#     p <- ggplot()+
#       geom_sf(data=coast,fill='gray50')+
#       geom_sf(data=d,aes(col=year))
#       # geom_path(data=d,aes(cogx,cogy,col=year),arrow=arrow(type = 'closed'))
#     print(p)
#   }
#   return(cog_ts)
# }
sable_cog <- calc_cog(sable_future,makeplot=T)
glimpse(sable_cog)
```
