...
# visualize
glorys_phy_bot %>% filter(longitude>-130) %>% ggplot(aes(longitude,latitude,color=bot))+geom_point()
...
# visualize
glorys_bgc_bot %>% filter(longitude>-130) %>% ggplot(aes(longitude,latitude,color=bot))+geom_point()
...
trawl_locs <- trawl %>% 
  ...
  dplyr::select(date,longitude_dd,latitude_dd,depth_trawl=depth) %>% 
  # convert to UTM10
...
```{r}
trawl <- trawl %>% 
  left_join(trawl_glorys_match,by=c("date"="date","longitude_dd","latitude_dd")) %>% 
  # thin columns to just the ones we (may) need for SDMs
  dplyr::select(date,year,species,scientific_name,cpue_kg_km2,lattrawlUTM,lontrawlUTM,depth_trawl=depth,area_swept_ha,glorys_match_bgc,glorys_match_phy)
```
...
join_glorys_trawl <- function(yr){
  ...
  out <- trawl_glorys_yr %>% 
    dplyr::select(date_trawl=date,year,species,scientific_name,cpue_kg_km2,lattrawlUTM,lontrawlUTM,depth_trawl,glorys_dttm,
                  depth_glorys=depth,glorys_time=time,longlorysUTM=lonUTM,latglorysUTM=latUTM,thetao,o2)
  out
}
```
...
```{r}
trawl_glorys_joined <- purrr::map_df(2003:2017,join_glorys_trawl)
