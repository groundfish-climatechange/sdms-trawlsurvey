...
footprints <- read_sf(here('data','DTS footprints','DTS_footprints_combined.shp')) %>% 
  ...
  mutate(port_name=case_when(
    ...
    port_group=="CLO" ~ "Astoria",
    port_group=="ERA" ~ "Eureka",
    port_group=="MRA" ~ "Morro Bay",
    port_group=="NPS" ~ "North Puget Sound",
  )) %>% 
  mutate(port_name=factor(port_name,levels=c("North Puget Sound","Astoria","Coos Bay","Eureka","Fort Bragg","Morro Bay")))

port_match <- tibble(port_name=c("Fort Bragg","Coos Bay","Astoria","Eureka","Morro Bay","North Puget Sound")) %>% 
