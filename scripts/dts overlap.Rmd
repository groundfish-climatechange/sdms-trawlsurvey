...
four_spp <- dover_ens %>% rename(dover=ens_est) %>% 
  ...
  dplyr::select(year,dover:ls_thornyhead,mean_bt_30d_ipsl,mean_oxy_bottom_30d_ipsl,depth_m,longitude,latitude) %>%
...
```{r}
library(ggnewscale)
aff_polys<-affinities_ellipses(four_spp,yr_vec=1980:2022,concavity=7,threshold=0.25)
...
polys_and_footprints <- ggplot()+
  ...
  scale_color_manual(values=c(pal4),name="75th quantile\nSpecies presence")+
  new_scale_color()+
  geom_segment(data=footprint_env_space,aes(x=mean_t_2020,y=mean_oxy_2020,xend=mean_t_2100,yend=mean_oxy_2100,
             col=port_name),size=1,linetype=1,arrow=arrow(type="open",length=unit(0.1,'inches')))+
...
fig2
ggsave(here('model output','dts paper','fishing_footprint_dumbbells_spp_affs.png'),fig2,h=8,w=8)
```
...
```{r}
fourspp_depth <- plot_depth_distribution2(four_spp,start_year = 2020,end_year=2100)
fourspp_depth
```
...
plt <- ggdraw()+
  ...
  draw_image(here('data','icons','dover1.jpg'),hjust=.24,vjust=-.4,scale=0.15)+
  draw_image(here('data','icons','sablefish.png'),hjust=-.26,vjust=-.4,scale=0.15)+
  draw_image(here('data','icons','shortspine.png'),hjust=.24,vjust=.15,scale=0.15)+
  draw_image(here('data','icons','longspine1.jpg'),hjust=-.26,vjust=.15,scale=0.15)
plt
...
partb <- plot_grid(multimap2020,NULL,multimap2100,rel_widths = c(1,-0.65,1),nrow=1)
...
ggsave(here('model output','dts paper','fig1.png'),w=6,h=8)
```
...
lic_footprints_all_plot2 <- lic_footprints_all %>% 
  ...
  mutate(anomaly=sum_lic-sum_lic[year==2020]) %>% 
  ungroup() %>%
  ggplot(aes(year-2000,anomaly,color=pair))+
...
lic_footprints_all_plot2
ggsave(here('model output','dts paper','footprints_lic_ts.png'),lic_footprints_all_plot2,h=4,w=6)
```
