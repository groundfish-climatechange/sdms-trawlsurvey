...
# get a sense for historical variability (95% quantiles?)
...
fig1
ggsave(here('model output','dts paper','fig1.png'),fig1,w=6.5,h=7)
```
...
dist_to_shore_fig <- ggdraw()+
  ...
  draw_line(x=c(0.27,0.37),y=c(0.84,0.85),size=1,lineend = "round",linetype=2)+
  draw_line(x=c(0.255,0.35),y=c(0.8,0.6),size=1,lineend = "round",linetype=2)+
  draw_line(x=c(0.29,0.35),y=c(0.76,0.7),size=1,lineend = "round",linetype=2)+
  draw_line(x=c(0.15,0.19),y=c(0.6,0.75),size=1,lineend = "round",linetype=2)+
  draw_image(here('data','icons','dover1.png'),hjust=.15,vjust=-.35,scale=0.08)+
  draw_image(here('data','icons','sablefish.png'),hjust=.15,vjust=-.12,scale=0.14)+
  draw_image(here('data','icons','shortspine.png'),hjust=.15,vjust=-.2,scale=0.12)+
  draw_image(here('data','icons','longspinet.png'),hjust=.35,vjust=-.09,scale=0.1)
...
depth_change_fig <- ggdraw()+
  ...
  # draw_line(x=c(0.22,0.35),y=c(0.89,0.9),size=1)+
  # draw_line(x=c(0.21,0.3),y=c(0.85,0.7),size=1)+
  # draw_line(x=c(0.245,0.3),y=c(0.82,0.77),size=1)+
  # draw_line(x=c(0.1,0.15),y=c(0.65,0.8),size=1)+
  draw_image(here('data','icons','dover1.png'),hjust=.35,vjust=-.34,scale=0.08)+
  draw_image(here('data','icons','sablefish.png'),hjust=.35,vjust=0,scale=0.14)+
  draw_image(here('data','icons','shortspine.png'),hjust=.35,vjust=-.09,scale=0.12)+
  draw_image(here('data','icons','longspinet.png'),hjust=.35,vjust=-.2,scale=0.1)
...
ggsave(here('model output','dts paper','fig_dist_depth_change2.png'),fig_dist_depth_change,w=8,h=7.5)
```
...
# mean across the entire ROMS domain
...
#   ggplot(aes(x=mean_t_hist,xend=mean_t_fut,y=fct_rev(port_name),col=port_name))+
#   geom_segment(col=,size=1,linetype=1,arrow=ggplot2::arrow(type="open",length=unit(0.1,'inches')))+
#   geom_segment(color="#FCAE91",colour_x =NA,colour_xend = "#A50F15",
...
fig3
ggsave(here('model output','dts paper','fig_env_change_env_affs.png'),fig3,height=9,width=8,units='in')
```
...
#   summarise(mean_change=mean(deltaL),upper=mean(deltaL)+sd(deltaL),lower=mean(deltaL)-sd(deltaL)) %>% 
#   filter(port_name%in%c("Astoria","Fort Bragg")) %>% 
#   ggplot(aes(port_name,mean_change,col=ESM,ymin=lower,ymax=upper))+
#   geom_pointrange(position=position_jitter())+
#   geom_hline(yintercept=0,linetype=2)+
#   scale_color_manual(values=c("#009175","#7E0018","#00C2F9"))+
#   # facet_wrap(~port_name,ncol=2)+
#   labs(x="Port Group",y=expression(paste(Delta,"L")))
#   # xlim(c(-0.03,0.03))+
...
ggsave(here('model output','dts paper','fig_cpue_bhat_fp.png'),dover_sable_cpue_overlap,h=8,w=6)
```
...
ggsave(here('model output','dts paper','fig_dts_landings_prop.png'),dts_prop_landings_plot,h=5,w=6)
```
...
ggsave(here('model output','dts paper','fig_dts_landings_lbs.png'),lbs_plot,h=4,w=6.5)
```
# Deprecated/Old
